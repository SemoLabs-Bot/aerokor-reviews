...
echo "== init run =="
RUN_JSON=$(node scripts/jira-voice/init-run.mjs --dev --source transcript --title "gen-dry" --transcriptText "$TRANSCRIPT")
echo "$RUN_JSON"
...
node scripts/jira-voice/generate-candidates.mjs --runFile "$RUN_FILE" --dryRun --maxCandidates 3

echo "== inspect run =="
RUN_ID="$RUN_ID" RUN_FILE="$RUN_FILE" node - <<'NODE'
const fs=require('fs');
